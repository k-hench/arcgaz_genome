...
data[c(1,3,4),] |> 
  ...
  scale_alpha_manual(values = c(`TRUE` = 1,
                                `FALSE` = .35),
                     guide = "none") +
  # scale_color_manual(values = clrs_n(7, c(clr_saturate(clrs), "gray20")[c(1,3,2)])) +
...
mhc |> 
  ...
  # scale_color_stepsn(colours = c("black", clrs[1], clr_lighten(clrs[2],.2)),
  #                    breaks = c(-Inf, -100, -50, -10, Inf)) +
...
read_busco_full <- \(gn){
  ...
  read_tsv(skip = 2, file) |> 
    mutate(genome = gn) |> 
    left_join(gn_plot |> rename(chr = "saccver")) |> 
    mutate(gstart = `Gene Start` + sstart,
           gend = `Gene End` + send,
           gmid = (gstart + gend) / 2)
}
```
...
```{r}
busco_summary <- genomes[c(1:3,6:8)] |> 
...
busco_summary |> 
  ...
  geom_text(aes(y = as.numeric(factor(genome)) - .5 +  lab_y, 
                ...
                color = after_scale(clr_darken(fill,.2))),
            family = fnt_sel) +
  geom_text(data = busco_summary |> 
              filter(abbrev == "C"),
            aes(y = as.numeric(factor(genome)), 
                x = .97,
                label = str_c(abbrev,": ", sprintf('%.1f', prct*100), "%")),
            family = fnt_sel,
  ...
  scale_fill_manual(values = clrs_n(4, c(clrs[2],"gray75", clrs[1]))) +
  # scale_fill_manual(values = scales::colour_ramp(c(clrs[2],"gray75", clrs[1]))((0:3)/3)) +
  coord_cartesian(xlim = c(-.01, 1.1),
                  ylim = c(0.5, 6.5),expand = 0) +
  theme_void(base_family = fnt_sel) +
  theme(legend.position = "none",
        axis.text.y = element_text(angle = 90),
        axis.ticks.x.bottom = element_line(color = "gray30", linewidth = .2, linetype = 1),
        axis.ticks.length.x.bottom = unit(-4,"pt"),
        axis.text.x = element_text(family = fnt_sel))
```


```{r}
#| eval: false
test <- read_busco_full(genomes[[6]])
```
